...
export default function GroupCreateForm({ categories }: { categories: AdminGroupCategory[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/bff/admin/groups", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setError(typeof first === "string" ? first : (body.message ?? "Could not create group"));
        setStatus("error");
        return;
      }

      router.push(`/console/groups/${body.id}`);
      router.refresh();
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <input id="name" name="name" required maxLength={30} className={inputClasses} />
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <select id="group_type" name="group_type" required defaultValue="" className={inputClasses}>
            <option value="" disabled>
              Select…
            </option>
      ...
      <div>
        ...
        <textarea id="description" name="description" rows={4} required maxLength={255} className={inputClasses} />
      </div>
