...
export default function PageCategoryCreateForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/bff/admin/page-categories", {
        method: "POST",
      ...
      (document.getElementById("page-category-form") as HTMLFormElement)?.reset();
      router.refresh();
    } finally {
      setStatus("idle");
    }
  ...
  return (
    <form id="page-category-form" onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      <input name="name" placeholder="Category Name" required className={inputClasses} />
      <input name="sort_order" type="number" min={0} placeholder="Sort Order" className={inputClasses} />
      <textarea name="description" placeholder="Description" rows={2} className={`sm:col-span-2 ${inputClasses}`} />
