...
export default function GuestEditForm({ guest }: { guest: AdminGuestDetail }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/bff/admin/guests/${guest.id}`, {
        method: "PUT",
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <input id="lastname" name="lastname" defaultValue={guest.lastname ?? ""} className={inputClasses} />
        </div>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <select id="gender" name="gender" defaultValue={guest.gender ?? ""} className={inputClasses}>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <select id="profession" name="profession" defaultValue={guest.profession ?? ""} className={inputClasses}>
            <option value="">—</option>
        ...
        <div>
          ...
          <input
            id="sub_occupation"
            name="sub_occupation"
            defaultValue={guest.sub_occupation ?? ""}
            className={inputClasses}
          />
        </div>
      </div>
      ...
      <LocationPicker
        initialCountryId={guest.country_id}
        initialStateId={guest.state_id}
        initialCityId={guest.city_id}
      />
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <input id="address" name="address" defaultValue={guest.address ?? ""} className={inputClasses} />
        </div>
        <div>
          <label htmlFor="pincode" className={labelClasses}>
            Postal Code
          </label>
          <input id="pincode" name="pincode" defaultValue={guest.pincode ?? ""} maxLength={10} className={inputClasses} />
        </div>
      </div>
