...
export default function MailingListCreateForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/bff/admin/mailing-lists", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setError(typeof first === "string" ? first : (body.message ?? "Could not create mailing list"));
        setStatus("error");
        return;
      }

      (document.getElementById("mailing-list-form") as HTMLFormElement)?.reset();
      router.refresh();
    } finally {
      setStatus("idle");
    }
  ...
  return (
    <form id="mailing-list-form" onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      <div>
      ...
      <div>
        ...
        <select id="scope" name="scope" defaultValue="subscription" className={inputClasses}>
      ...
      >
        {status === "submitting" ? "Adding…" : "Add Mailing List"}
      </button>
    </form>
  );
}
