...
import type { AdminGalleryDetail } from "@/lib/api-types";
...
export default function GalleryEditForm({ gallery }: { gallery: AdminGalleryDetail }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/bff/admin/gallery/${gallery.id}`, {
        method: "PUT",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <input id="name" name="name" defaultValue={gallery.name} required className={inputClasses} />
      </div>
