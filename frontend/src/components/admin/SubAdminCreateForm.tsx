...
export default function SubAdminCreateForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/bff/admin/subadmins", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setError(typeof first === "string" ? first : (body.message ?? "Could not create sub-admin"));
        setStatus("error");
        return;
      }

      router.push(`/console/subadmins/${body.id}`);
      router.refresh();
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      >
        {status === "submitting" ? "Creating…" : "Create Sub-Admin"}
      </button>
    </form>
  );
}
