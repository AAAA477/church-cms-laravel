...
<div class="w-full max-w-4xl">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="px-6 py-4 border-b border-gray-100">
            <p class="text-sm text-gray-500">Edit the HTML / CSS / JS content for this widget. Changes will be reflected on the public site.</p>
        </div>

        <form method="post" action="{{ url('/admin/widgets/' . $editInfo->id . '/update') }}" id="widgets" enctype="multipart/form-data">
            @csrf

            @if (old('content') != '')
                @php $content = old('content'); @endphp
            @else
                @php $content = $editInfo->content; @endphp
            @endif

            <div class="px-6 pt-5 pb-2 grid grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-medium text-gray-700 block mb-1">Page</label>
                    <select name="page" class="tw-form-control w-full">
                        <option value="home" {{ old('page', $editInfo->page) === 'home' ? 'selected' : '' }}>Home</option>
                    </select>
                </div>
                <div>
                    <label class="text-sm font-medium text-gray-700 block mb-1">Display Order</label>
                    <input type="number" name="display_order" value="{{ old('display_order', $editInfo->display_order) }}" min="0" class="tw-form-control w-full">
                </div>
            </div>
...
@push('scripts')
    ...
    <style>
        .CodeMirror { height: 420px; font-size: 13px; font-family: 'JetBrains Mono', 'Fira Code', monospace; }
    </style>
    <script>
